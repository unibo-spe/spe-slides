...
- As opposed to __general-purpose languages__ (GPL) which are targeting __as many__ classes of problems __as possible__
...
## Running example: the `Scheduler` DSL (pt. 1)

> __Scheduler__ $\equiv$ **Sh**ell + Sch**eduler**
>
> ¯\\_(ツ)_/¯
...
## Running example: the `Scheduler` DSL (pt. 2)

Code: <https://github.com/unibo-spe/scheduler-lang>
...
5. You may also use __IntelliJ__, in that case just import the `scheduler-lang/` directory as _Gradle project_
...
│               └── it/unibo/spe/mdd/scheduler/
│                   └── scheduler
│                       ├── GenerateScheduler.mwe2
│                       └── Scheduler.xtext
├── it.unibo.spe.mdd.scheduler.ide/
│   └── build.gradle
├── it.unibo.spe.mdd.scheduler.web/
│   └── build.gradle
└── settings.gradle
```
...
- `scheduler` project is where the domain is modelled, and the language is defined
    * including parser, validator, scoping, etc.
    * 2 very important files:
        + `Scheduler.xtext`: this is where modelling and language definition occurs
        + `GenerateScheduler.mwe2`: this is where the automated generation of scoping, validation, generation, testing facilities is configured
- `ide` project is where the generic IDE support via LSP is defined
    * depends on `scheduler` project
...
- `jettyRun` starts the Web-playground for the Scheduler language
...
## The `GenerateScheduler.mwe2` file

- This is automatically generated by Eclipse when setting up an Xtext project
- Pretty self-explanatory:

    ```kotlin
    module it.unibo.spe.mdd.scheduler.GenerateScheduler
        ...
        component = XtextGenerator {
            configuration = {
                project = StandardProjectConfig {
                    baseName = "it.unibo.spe.mdd.scheduler"
                    rootPath = rootPath
            ...
            language = StandardLanguage {
                name = "it.unibo.spe.mdd.scheduler.Scheduler"
                fileExtensions = "shed"
...
```groovy
grammar it.unibo.spe.mdd.scheduler.Scheduler with org.eclipse.xtext.common.Terminals

generate scheduler "http://www.unibo.it/spe/mdd/scheduler/Scheduler"
...
interface SchedulerFactory  {
  ...
  + getSchedulerPackage(): SchedulerPackage
...
- Validation rules are defined in the `SchedulerValidator` class
    * package: `it.unibo.spe.mdd.scheduler.validation`
    * Gradle sub-project: `scheduler-lang/`__`it.unibo.spe.mdd.scheduler`__

- Stub class is generated by Xtext when running the `generateXtextLanguage` task
    * which simply triggers the execution of the `GenerateScheduler.mwe2` file

- Content of the stub and validation rule example:
    ```java
    package it.unibo.spe.mdd.scheduler.validation;
    import it.unibo.spe.mdd.scheduler.scheduler.*;
    import org.eclipse.xtext.validation.Check;
    import org.eclipse.xtext.validation.CheckType;

    public class SchedulerValidator extends AbstractSchedulerValidator {
        @Check(CheckType.FAST)
        public void ensureDateIsValid(Date date) {
            if (date.getYear() < 0) {
                error("Year must be positive", date, SchedulerPackage.Literals.DATE__YEAR, 0);
            }
            if (date.getMonth() < 1 || date.getMonth() > 12) {
                error("Month must be between 1 and 12", date, SchedulerPackage.Literals.DATE__MONTH, 0);
            }
            if (date.getDay() < 1 || date.getDay() > 31) {
                error("Day must be between 1 and 31", date, SchedulerPackage.Literals.DATE__DAY, 0);
            }
        }
    }
    ```
...
- Validation rules are defined in the `SchedulerScopeProvider` class
    * package: `it.unibo.spe.mdd.scheduler.scoping`
    * Gradle sub-project: `scheduler-lang/`__`it.unibo.spe.mdd.scheduler`__
...
- Content of the stub and validation rule example:
    ```java
    public class SchedulerScopeProvider extends AbstractSchedulerScopeProvider {
        @Override
...
3. Design insights:
    1. we may define some _custom_ notion of `SchedulerTask` encapsulating:
    ...
    2. we may define some custom notion of `SchedulerRuntime`
        + leveraging on the `ScheduledExecutorService` API...
        + ... to schedule the `SchedulerTask`s for execution

---

## Example: the `SchedulerRuntime` and `SchedulerTask` classes
...
public class SchedulerRuntime {
    ...
    public SchedulerRuntime(ScheduledExecutorService delegate) {
...
public class SchedulerTask {
    private SchedulerTask(String name, String command, String entrypoint, Duration delay) { /*...*/ }
    ...
    public static SchedulerTask in(String name, String command, String entrypoint, Duration delay) { /*...*/ }
    public static SchedulerTask in(String command, String entrypoint, Duration delay) { /*...*/ }

    public static SchedulerTask at(String name, String command, String entrypoint, LocalDateTime dateTime) { /*...*/ }
    public static SchedulerTask at(String command, String entrypoint, LocalDateTime dateTime) { /*...*/ }
}
```
{{% /col %}}
{{% /multicol %}}
...
public static void main(String[] args) {
    SchedulerRuntime runtime = new SchedulerRuntime(Executors.newScheduledThreadPool(1));
    pool_myPool(runtime);
    pool_otherPool(runtime);
}

private static void pool_myPool(SchedulerRuntime runtime) {
    SchedulerTask task0 = SchedulerTask.in("greetFrequently", "echo hello", "/bin/sh -c", Duration.parse("PT5M"));
    task0.setPeriodic(Duration.parse("PT1H"));
    runtime.schedule(task0);
    SchedulerTask task1 = SchedulerTask.at("greetOnce", "echo hello", "/bin/bash -c", LocalDateTime.parse("2030-10-11T12:13"));
    runtime.schedule(task1);
}

private static void pool_otherPool(SchedulerRuntime runtime) {
    SchedulerTask task0 = SchedulerTask.in("shutdownAfter1Day", "sudo shutdown now", "/bin/zsh -c", Duration.parse("PT24H"));
    runtime.schedule(task0);
}
```
{{% /col %}}
...
## Exercise 3: write a code generator for the `Scheduler` DSL

- The code generator should output code having a structure similar to the one shown in the previous slide

- The `SchedulerRuntime` and `SchedulerTask` classes may be generated as they are in the example

- The key part is generating the _main_, where components are assembled together

```java
public class SchedulerGenerator extends AbstractSchedulerGenerator {
    @Override
    public void doGenerate(Resource resource, IFileSystemAccess2 fsa, IGeneratorContext context) {
        ...
        fsa.generateFile("SchedulerRuntime.java", "CODE HERE");
        fsa.generateFile("SchedulerTask.java", "CODE HERE");
        fsa.generateFile("SchedulerSystem_" + inputFileName + ".java", "CODE HERE");
    }
}
```
...
## Exercise 4: write an interpreter for the `Scheduler` DSL

- No code generation, just a `main`
    1. parsing the DSL
    2. and converting each `Task` into a `SchedulerTask`
    3. and running each task via some `SchedulerRuntime`

- The `SchedulerRuntime` and `SchedulerTask` classes should be part of the runtime

- The key part is writing the `main`:

```java
public class SchedulerInterpreter {
    public static void main(String[] args) {
        if (args.length == 0) {
            System.err.println("Aborting: no path to .shed file provided!");
            return;
        }
        Injector injector = new SchedulerStandaloneSetup().createInjectorAndDoEMFRegistration();
        SchedulerInterpreter interpreter = injector.getInstance(SchedulerInterpreter.class);
    ...
    protected void runFile(String string) {
        ...
        for (TaskPool pool : taskPools.getPools()) {
            for (Task task : pool.getTasks()) {
                runtime.schedule(toSchedulerTask(task));
            }
        }
    }

    private SchedulerTask toSchedulerTask(Task task) {
        // TODO
    }
}
```
